...
var (
	...
	slp081ReactImportPattern    = regexp.MustCompile(`(?ms)^\s*import\s+(?:[^;]*?\s+from\s+)?["']react["']\s*;?\s*$`)
)
...
func slp081HasReactImport(content string) bool {
	if strings.HasPrefix(strings.ToLower(strings.TrimSpace(content)), "import type ") {
		return false
	}
